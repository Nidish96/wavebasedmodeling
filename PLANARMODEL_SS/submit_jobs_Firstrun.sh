...
for SWP in up; do
    for FMP in 9; do
        for WFRC in 250; do
            ...
            # else
                sbatch --time=00:00:10 --export=ALL,SMP=$SMP,EPT=$EPT,DOF=\'$DOF\',FMP=$FMP,WFRC=$WFRC,SWP=\'$SWP\',TTOT=$TTOT --job-name="$EPT$DOF-$FMP-$WFRC-$SWP" run_setup_FirstStudy.slurm
                echo "Will Create ./DATS/FIRST_1MAR21/RESU${SMP}_PT${EPT}${DOF}_F${FMP}000_W${WFRC}_Sweep${SWP}.mat"
